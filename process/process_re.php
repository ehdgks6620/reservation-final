...
if($max>9){
    ...
    $sql = "select * from r_table where re_name = '{$_POST['re_name']}' and re_univ = '{$_POST['re_univ']}' and re_num = '{$_POST['re_num']}'";
    $check = mysqli_fetch_array(mysqli_query($connect,$sql));
    if(strlen($_POST['re_name'])<15 && strlen($_POST['re_name'])>3 &&strlen($_POST['re_univ'])<30  && strlen($_POST['re_univ'])>=9 && strlen($_POST['re_num'])<=11 && strlen($_POST['re_num'])>9 ){
        if(isset($check['re_name'])){
            ...
            $sql = "insert into r_table(re_pro, re_name, re_time, re_univ, re_num) values ('{$_POST['re_pro']}','{$_POST['re_name']}','{$_POST['re_time']}','{$_POST['re_univ']}','{$_POST['re_num']}')";
            $result = mysqli_query($connect,$sql);
            if($result === false){
